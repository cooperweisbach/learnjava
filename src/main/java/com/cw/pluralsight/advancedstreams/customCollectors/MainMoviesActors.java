...
//                .forEach(entry -> System.out.println(entry.getKey() + "-" + entry.getValue()));
        ...
        Supplier<Map<Actor, Long>> supplier = HashMap::new;
        ...
        Supplier<HashMap<Actor, AtomicLong>> supplier1 = HashMap::new;
        BiConsumer<HashMap<Actor, AtomicLong>, Movie> accumulator1 = (map, movie) -> movie.getActors().forEach(
        ...
        movies.stream()
                .collect(
                        Collectors.groupingBy(
                                Movie::getReleaseYear,//Key Extractor
                                Collector.of(
                ...
                .collect(Collectors.toMap(
                        ...
                        entry -> entry.getValue().entrySet().stream()
                                .max(
                                        Map.Entry.comparingByValue(Comparator.comparing(AtomicLong::intValue))
                                ).get()
                ));
